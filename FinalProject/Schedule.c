...
{
	pSchedule->appointments = (Appointment*)malloc(11 * sizeof(Appointment));
...
int loadScheduleFromTextFile(Schedule* pSchedule, FILE* file, Barber* pBarber)
{
	...
	pSchedule->appointments = (Appointment*)malloc(MAX_APPOINTMENTS * sizeof(Appointment));
	...
	{
		Appointment* appointment = &pSchedule->appointments[0];
